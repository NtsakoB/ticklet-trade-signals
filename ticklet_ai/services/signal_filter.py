from ticklet_ai.services.rideout_policy import policy
def rideout_should_alert(*, price_now, entry_low, entry_high, rr_tp2,
                         late_p=None, extend_p=None, reentry_p=None,
                         overext_atr=None):
    if not (entry_low <= price_now <= entry_high): return False
    if rr_tp2 < policy.min_rr_tp2: return False
    ...
    if overext_atr is not None and overext_atr > policy.max_overextension_atr: return False
    return True