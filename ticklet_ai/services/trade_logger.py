...
from .features import build_from_result
...
def log_open(trade_id: str, symbol: str, strategy: str, side: str, timeframe: str, entry: float, result: Dict[str,Any]):
    feats = build_from_result(symbol, timeframe, strategy, result)
    rec = {"trade_id": trade_id, "ts_open": _ts(), "ts_close": "", "symbol": symbol, "strategy": strategy,
           "side": side, "timeframe": timeframe, "entry": entry, "exit": "", "hold_minutes": "",
           "pnl_pct": "", "win": "", **{k:feats[k] for k in feats if k not in ("ts_utc","symbol","timeframe","strategy")}, "raw": result}
    upsert_trade(rec)
...
def log_update(trade_id: str, updates: Dict[str, Any]):
    upsert_trade({"trade_id": trade_id, **updates})
...
def log_close(trade_id: str, exit_price: float, hold_minutes: int, pnl_pct: float):
    ...
    upsert_trade({"trade_id": trade_id, "ts_close": _ts(), "exit": exit_price, "hold_minutes": hold_minutes, "pnl_pct": pnl_pct, "win": win})
