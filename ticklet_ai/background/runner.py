        ...
        for sname in list_strategies():
            ...
            for sym in symbols:
                for tf in tfs:
                    ...
                    sig = {
                      "ts_utc": ts,"symbol": sym,"timeframe": tf,"strategy": sname,
                      ...
                      "tp1": result.get("tp1") or "","tp2": result.get("tp2") or "","tp3": result.get("tp3") or "",
                      "confidence": result.get("confidence") or result.get("ai_confidence") or "",
                      "anomaly": result.get("anomaly") or "","pump_conf": result.get("pump_confidence") or "",
                      "raw": result,
                    }
                    upsert_signal(sig)
                    feats = build_from_result(sym, tf, sname, result)
                    if not feats.get("ts_utc"): feats["ts_utc"] = ts
