...
from ..services.scanner import get_candidates
from ..services.notifier import send_trade, send_maint
...
def generate_signal():
    ...
    for c in top:
        ok = rideout_should_alert(price_now=c["price_now"],
        ...
        if ok:
            ...
            send_trade(msg)
            emitted.append(c["symbol"])
        else:
            missed.append(c["symbol"])
            send_maint(f"Missed: {c['symbol']} (entry window/AI/TP2/volume gates failed)")
