...
export function LiveStatsCards({ refreshInterval = 30000 }: { refreshInterval?: number }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard title="Active Signals" value={String(stats.active_signals)} description="Live trading signals"
          chart={stats.performance_history.slice(-10).map(p => ({ value: p.trades_count }))}
          trend={stats.active_signals > 5 ? "up" : stats.active_signals > 0 ? "neutral" : "down"} isLive />
        <StatCard title="Executed Trades" value={String(stats.executed_trades)} description="Total completed trades"
          chart={stats.performance_history.slice(-10).map(p => ({ value: p.trades_count }))} trend="neutral" />
        <StatCard title="Win Rate" value={`${winRatePct}%`} description="Overall success rate"
          chart={stats.performance_history.slice(-10).map(p => ({ value: p.win_rate * 100 }))}
          trend={stats.win_rate > 0.6 ? "up" : stats.win_rate > 0.4 ? "neutral" : "down"} />
        <StatCard title="Capital at Risk" value={`$${stats.capital_at_risk.toLocaleString()}`} description="Current exposure"
          chart={stats.performance_history.slice(-10).map(p => ({ value: p.balance / 100 }))}
          trend={stats.capital_at_risk > stats.total_balance * 0.5 ? "down" : "neutral"} />
      </div>
        ...
        <CardContent className="pt-6">
          <div className="flex justify-between items-center">
            <div>
              ...
              <h3 className="text-3xl font-bold text-white">${stats.total_balance.toLocaleString()}</h3>
              <p className="text-xs text-muted-foreground mt-1">
                {stats.total_balance > stats.starting_balance ? '+' : ''}
                ${(stats.total_balance - stats.starting_balance).toLocaleString()} (
                {((stats.total_balance - stats.starting_balance) / (stats.starting_balance || 1) * 100).toFixed(1)}%)
              </p>
            </div>
            <div className="h-16 w-32">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={stats.performance_history.slice(-20)}>
                  <Line type="monotone" dataKey="balance" stroke={stats.total_balance > stats.starting_balance ? "#10b981" : "#ef4444"} strokeWidth={2} dot={false}/>
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
